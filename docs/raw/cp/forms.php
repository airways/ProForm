    ...
    <tbody>
        ...
        <tr>
            ...
            <td>This option can be set to the name of a field, the value of which will be used as the notification's email address in the Reply-To header. This means that any recipient of the notification will automatically be able to reply to the email address entered into this field in the form.<br/><br/>
                For instance, often times this option would be set to a field filled out by the visitor such as <b>my_email_address</b>. Whatever value is entered by the user would then be set as the Reply-To for the notifications. Assuming that the Notification List is sent to a customer service rep, or other person who would like to easily contact the submitter of the form, they can simply use their email client's <b>Reply</b> command to compose a new return message directly to the visitor.</td>
        </tr>
    </tbody>
</table>
<h4>
...
<p>These notification settings are useful in a situation where you want to provide a pre-set list of available contact addresses, and ask the user to pick one of these options. By specifying the field which contains these options as the Notification Field Setting's <b>Email Field</b> option, you will then get a notification sent only to that individual address.</p>

<p>This is different from the Notification List settings above, in that the entire list of addresses specified on the Notification List always receive the notification - while there is only ever <b>one</b> recipient of a notification sent from a Notification Field. Another difference is that the Notification List uses a static list of addresses defined on the form's settings, while Notification Fields take their options form a field instead.</p>
...
<p>After adding a heading, it can be moved around the form layout similarly to a field. Simply click the heading to select it, then drag to move it to a new position. To edit a heading, click its <b>Edit</b> link after selecting it.</p>
...
<p><dfn>ProForm</dfn> profiles the capability to build multistep forms. Separating a form into multiple steps is extremely simple, and very similar to use the previously discusses headings.</p>

<p>Adding a step to a form is similar to adding a heading or a field. To add a new step in a form, first click the form's <b>Edit Layout</b> link from the form listing (or click the form's <b>Layout</b> tab if you already have the form open). Then, scroll down in the <kbd>Toolbox</kbd> to the <kbd>Special</kbd> section and click the <b>Step</b> button.</p>

<p>After adding a step, it can be moved around the form layout similarly to a field. Moving the step around the layout splits the form into different sized steps. To move a step (and therefore move fields between them into different steps), simply click the step to select it, then drag to move it to a new position. To edit a step, click its <b>Edit</b> link after selecting it.</p>
